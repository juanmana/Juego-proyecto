class Obstacles {
  ...
  generateObstacles() {
    if (this.listObstacles.length < 7) {
      ...
      if (this.canRoad(obstacle)) this.listObstacles.push(obstacle);
    }
  }
  ...
  canRoad(obstacle) {
    for (var i = 0; i < this.listObstacles.length; i++) {
      if (
        obstacle.posY == this.listObstacles[i].posY &&
        obstacle.orientation != this.listObstacles[i].orientation
      )
        return false;
      ...
      if (
        obstacle.posY == this.listObstacles[i].posY &&
        obstacle.orientation == this.listObstacles[i].orientation &&
        obstacle.speed > this.listObstacles[i].speed
      )
        return false;
    }
    return true;
