class Game {
  ...
  isTrunk2() {
    ...
    if (this.player2.posY < 240 && !this.player.onTheMove) {
      this.trunks.forEach(trunk => {
        if (
          this.player2.posX < trunk.posX + 250 &&
  ...
  meta() {
    if (
      this.player.posX < this.home.posX + 70 &&
      ...
      if (
        this.player.posY == 40 &&
        this.player.posX <= 440 &&
        this.player.posX >= 400
      )
        this.player.goals++;
  ...
  isTrunk() {
    ...
    if (this.player.posY < 240 && !this.player.onTheMove) {
      this.trunks.forEach(trunk => {
        if (
          this.player.posX < trunk.posX + 250 &&
  ...
  isCollision() {
    for (var i = 0; i < this.obstacles.listObstacles.length; i++) {
      if (
        this.obstacles.listObstacles[i].posX <
          this.player.posX + this.player.width &&
        this.player.posX <
          this.obstacles.listObstacles[i].posX +
            this.obstacles.listObstacles[i].width &&
        this.obstacles.listObstacles[i].posY <
          this.player.posY + this.player.height &&
        this.player.posY <
          this.obstacles.listObstacles[i].posY +
            this.obstacles.listObstacles[i].height
      ) {
        this.player = new Player(
          this,
          360,
      ...
      if (
        this.obstacles.listObstacles[i].posX <
          this.player2.posX + this.player2.width &&
        this.player2.posX <
          this.obstacles.listObstacles[i].posX +
            this.obstacles.listObstacles[i].width &&
        this.obstacles.listObstacles[i].posY <
          this.player2.posY + this.player2.height &&
        this.player2.posY <
          this.obstacles.listObstacles[i].posY +
            this.obstacles.listObstacles[i].height
      ) {
